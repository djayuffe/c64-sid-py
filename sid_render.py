...
def read_file(path: str) -> bytes:
    """Read a binary input file while preserving its actionable OS error."""
    return Path(path).read_bytes()


def main() -> int:
    ap = argparse.ArgumentParser(description='Render a PSID or RSID tune to mono PCM WAV.')
    ap.add_argument('--version', action='version', version=f'c64sid {__version__}')
    ap.add_argument('input_sid', help='Input PSID or RSID file')
    ap.add_argument('output_wav', help='Output mono 16-bit PCM WAV file')
    ap.add_argument('--seconds', type=float, default=10.0, help='Finite render duration in seconds (default: 10)')
    ap.add_argument('--rate', type=int, default=44100, help='Output sample rate, 8000-96000 Hz (default: 44100)')
    ap.add_argument('--song', type=int, help='One-based subsong number (default: SID file start song)')
    ...
    try:
        ...
        if args.dump_sidpro:
            pb.enable_sidpro_export(
                ...
                telemetry_rate=args.sidpro_telemetry_rate,
                verbose_sid=not bool(args.no_sidpro_verbose_sid),
                io_trace=not bool(args.no_sidpro_io_trace),
            )

        pb.load_sid_bytes(raw, song=args.song)
        res = pb.render_to_wav(args.output_wav, seconds=args.seconds, sample_rate=args.rate)
    except (OSError, RuntimeError, ValueError) as error:
        print(f'Error: {error}', file=sys.stderr)
        return 2

    SystemLogger.log('CLI', f"Wrote {res.wav_path} | song: {pb.song} | rendered play calls: {res.frames_rendered} | samples: {res.samples}", 'info')
    return 0
