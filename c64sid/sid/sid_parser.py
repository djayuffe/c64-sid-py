...
def parse_sid_header(raw: bytes) -> Tuple[SidHeader, bytes]:
    ...
    if data_offset < 0x76 or data_offset > len(raw):
        raise ValueError(f'Invalid SID file: data offset {data_offset:#x} is outside the file')
    if songs == 0 or start_song == 0 or start_song > songs:
        raise ValueError('Invalid SID file: start song must be within the declared song count')
